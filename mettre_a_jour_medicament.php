...
<head>
    ...
    <title>Mettre à jour un medicament</title>
    <style>
        .backtomenu {
            display: flex;
        }
...
<body><nav class="navbar navbar-expand-lg bg-body-tertiary">
    ...
    <div class="profile-container">
        <div class="retour">
            <a href="affichermedicaments.php" class="btn btn-primary"> <h4>Medicaments <<<</h4></a><h4> Mise à jour d'un medicament</h4>
        </div>
        <?php        
            $mise_a_jourID = $_GET['mettre_a_jourID'];
            $sql="select nom from `medicament` where medicamentID = $mise_a_jourID";
            ...
            if(isset($_POST['submit'])){
                ...
                $verifsql = "select nom from `medicament` where nom = '$nom' and medicamentID <> $mise_a_jourID";
                $verifresult = mysqli_query($con, $verifsql);

                if($verifresult){
                    if($verifresult -> num_rows ==0) {
                        $sql = "update `medicament` set nom='$nom' where medicamentID='$mise_a_jourID'";
                        $result=mysqli_query($con, $sql);
                        if($result){
        ...
        <div class="profile-div" id="profile-div">
            <form method="post" class="profile-form">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="nom" name="nom" value="<?php echo $mise_a_jourNom ?>" required>
                </div>
